...
module.exports = screens = (grunt) ->
  ...
  build: (fn) ->
    ...
    if screens?.ios?.iphonePortrait736hx3?
      grunt.file.copy screens.ios.iphonePortrait736hx3, path.join(res, 'Default-736h.png'), encoding: null

    if screens?.ios?.iphoneLandscape736hx3?
      grunt.file.copy screens.ios.iphoneLandscape736hx3, path.join(res, 'Default-Landscape-736h.png'), encoding: null
